...
from const import alignmethod
import dpalign as dp
import numpy as np
import pprint
from dpalign import plot_S
...
def fast_align(data, ladders, peaks , qcfunc ):
    ...
    print(' -> HQ peak: %d / %d' % (len(hq_peaks), len(peaks)))

    hq_score = mq_score = -1

    if len(hq_peaks) > 2 and ( len(hq_peaks) >= len(ladders) - 1 or len(peaks) == len(ladders) ):
        print(' -> fast_align() HQ peak: %d => ' % len(hq_peaks))
        hq_result = do_fast_align( hq_peaks, ladders, peaks )
        (hq_score, hq_msg) = qcfunc( hq_result, method = 'strict' )
        print('%d | %4.2f | %6.2f | %3.2f' %
    ...
    if len(mq_peaks) > len(hq_peaks) and len(mq_peaks) >= len(ladders) - 1:
        print(' -> fast_align() MQ peak: %d => ' % len(mq_peaks))
...
def greedy_align( data, ladders, peaks, qcfunc ):
    ...
    for i in range(0, I):
        for j in range(1, J):
            ...
            # print(type(initial_peak_pairs[0]))
            ...
            print(* zip(*peak_pairs))

    dur = time.perf_counter() - start
    print("time of peak to pairs is {:.2f}s".format(dur))
    ...
    for (rss, peak_pairs, z, initial_peak_pairs) in seed_peek_pairs:
        ...
        print('Current best score: %3.2f dp: %3.2f rss: %4.2f' % (score, dp_score, dp_rss))
        print('Start aligning by shifting pairs...')
...
def progressive_align( peaks, ladders, peak_pairs ):
    ...
    print(' progressive_align() ==> %4.2f | %5.2f | %d' % (dp_score, dp_rss, len(dp_peaks)))
...
def estimate_peak_pairs( peaks, ladders, z ):
    ...
    # generate synthetic standard peaks 生成合成标准峰
    standard_peaks = []
    f = np.poly1d(z)
    ...
    for (rtime, ladder) in standard_peaks:
        ...
        aln_peak = peaks[0]
        for p in peaks[1:]:
            c = abs( rtime - p.rtime ) ##合成标准峰rtime-实际峰rtime
            if c < min_dev:
                min_dev = c
                aln_peak = p
